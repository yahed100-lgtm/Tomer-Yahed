...
**Online EWC:** Fisher matrices from successive tasks are combined via a running average (50/50 blend) rather than stored separately, avoiding parameter bloat across many tasks  
...
**EWC implementation:** `ConsolidatedEWC` — Fisher matrices are *accumulated* (added together) across tasks rather than averaged, so earlier tasks maintain persistent influence  
...
```bash
# Figure 2A — fast, good starting point (~minutes on CPU)
...
**Online EWC (`main-graph A.py`):** Rather than accumulating a separate Fisher matrix per task, this implementation maintains a single running-average Fisher (50/50 blend at each task boundary). This avoids memory growth with more tasks while preserving knowledge across the task sequence.

**Consolidated EWC (`main-graph_B.py`):** Uses additive Fisher accumulation — each new task's Fisher is added on top of the existing total. This means earlier tasks exert a stronger cumulative restraint on parameters, matching the formulation in the original paper more closely.
...
**Online EWC:** מטריצות הפישר ממשימות עוקבות ממוצעות (50/50) במקום לשמור אותן בנפרד — חוסך זיכרון לאורך משימות רבות  
...
| EWC                   | MLP                  | פנלטי פישר מצטבר (חיבורי) על כל המשימות |

**ארכיטקטורה:** MLP עם רוחב **1500** (רחב בהרבה מ-Figure 2A)  
**מימוש EWC:** `ConsolidatedEWC` — מטריצות Fisher **מצטברות** (חיבור) בין משימות, כדי שמשימות ישנות ישמרו השפעה קבועה  
...
```bash
# Figure 2A — מהיר, נקודת התחלה טובה (~דקות על CPU)
python "main-graph A.py"
...
> **GPU מומלץ** עבור `main-graph_B.py` ו-`main-GraphC.py`. שני הסקריפטים מזהים CUDA אוטומטית ומשתמשים בו אם זמין.
...
**Online EWC (`main-graph A.py`):** במקום לצבור מטריצת Fisher נפרדת לכל משימה, המימוש שומר ממוצע רץ (50/50) אחד משותף. זה מונע גידול בצריכת הזיכרון עם מספר המשימות תוך שמירה על הידע הצבור.

**Consolidated EWC (`main-graph_B.py`):** Fisher **מצטבר** (חיבורי) בין משימות — כל task חדש מוסיף את ה-Fisher שלו על גבי הסך הקיים. כך משימות ישנות ממשיכות להשפיע על האימון לאורך כל הרצף, בהתאם לנוסחה המקורית במאמר.

**אמידת Fisher (`main-GraphC.py`):** משתמש ב-`batch_size=1` (גרדיאנטים פר-דוגמה) ולא בגרדיאנטים ממוצעים של batch. זה מייצר אמידה מדויקת יותר של Fisher האלכסוני האמיתי, כיוון שריבוע גרדיאנט ממוצע מעריך בחסר את חשיבות הפרמטרים.
